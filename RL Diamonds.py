...
def GetMissionXML():
    ...
    # specify diamond spawn location
    diamond = str()
    for i in range (-SIZE,SIZE,1):
        for j in range(-SIZE,SIZE,1):
            if random.random() <= REWARD_DENSITY:
                diamond += f"<DrawBlock x='{i}'  y='2' z='{j}' type='diamond_ore'/>"

    # specify lava spawn location
    lava = str()
    for i in range (-SIZE,SIZE,1):
        for j in range(-SIZE,SIZE,1):
            if random.random() <= PENALTY_DENSITY:
                lava += f"<DrawBlock x='{i}'  y='1' z='{j}' type='lava'/>"
